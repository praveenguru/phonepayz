...
class MobileSuperDistributor extends StatefulWidget{
  @override
  _MobileSuperDistributorState createState() => _MobileSuperDistributorState();
}

class _MobileSuperDistributorState extends State<MobileSuperDistributor> {
  String token = "";
  ...
  addMoney(String id){
    ...
    ApiProvider().addMoney(token, int.parse(amount), id).then((response) async{
      if(response.status){
        await getSuperDistributors(token);
  ...
  //getContentWidget
  getContentWidget(){
    return (superDistributors.isEmpty)?Container():ListView.builder(
      itemCount: superDistributors.length,
      shrinkWrap: true,
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        FlatButton(
                            onPressed: (){
                              showDialogBox(superDistributors[index].id);
                            },
                            shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(4),
                        ...
                        SizedBox(height: 4,),
                        Text("\u20B9 "+superDistributors[index].balance.toString(),style: TextStyle(fontSize: 14,color: Colors.red.shade500),),
                      ],
                    )
                  ],
                )
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          elevation: 1,
          centerTitle: true,
          title: Text("Super Distributor",style: TextStyle(fontWeight: FontWeight.bold,color: Colors.white),),
        ),
        floatingActionButton: FloatingActionButton(
          elevation: 0,
          backgroundColor: Constants().primaryColor,
          onPressed: () async{
            await Navigator.push(context, FadeTransitionPageRouteBuilder(page: MobileCreateSuperDistributor()));
            getSuperDistributors(token);
          },
